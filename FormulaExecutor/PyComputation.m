...
+(void) initialize
{
    ...
    url = [url URLByAppendingPathComponent: @"Python" isDirectory:YES];
    NSAssert(url, @"Need a valid cache path");
    NSAssert(url.isFileURL, @"Need a valid cache path");
    [[NSFileManager defaultManager] createDirectoryAtURL: url withIntermediateDirectories:YES attributes:nil error:nil];
    
    setenv( "PYTHONPYCACHEPREFIX", url.fileSystemRepresentation, true);
}
...
- (BOOL)startInterpreter:(NSError**)errorPtr
{
	...
	if (!geometryModule) {
		...
		Py_DECREF( _globals); _globals = NULL;
		Py_Finalize();
		[gPyLock unlock];
		return NO;
	}
	...
	PyDict_SetItemString(_globals, "geometry", geometryModule);	// install as geometry.x -- NOTE: THIS IS BORROWED,

	// Install built-in functions at builtin level
	PyDict_SetItemString(_globals, "__builtins__", PyEval_GetBuiltins( ));// ref was borrowed
    ...
    for ( NSString *libName in requiredLibraries) {
        ...
        PyDict_Merge(_globals, PyModule_GetDict(module), 0);
        Py_DECREF( module);
    }
...
- (NSError*)errorFromPythonException: (PyObject *)exception
{
	...
	if (PyErr_GivenExceptionMatches( exception, PyExc_SyntaxError)) {
		...
		// try to figure out what the name was, sure would be easier if it weren't a UNADORNED STRING!
...
- (id)valueWithPythonResult:(PyObject*)result error:(NSError *__autoreleasing *)errorPtr
{
    ...
    if (PyUnicode_Check(result))
    ...
    else if (PyDateTime_Check(result)) {
        ...
        components.hour=PyDateTime_DATE_GET_HOUR(result);
        components.minute=PyDateTime_DATE_GET_MINUTE(result);
        components.second=PyDateTime_DATE_GET_SECOND(result);
...
- (id) calculateForInfo:(NSDictionary*)symbolTable withError:(NSError**)errorPtr
{
	...
	PyObject* result = PyEval_EvalCode( _code, _globals, locals);
	PyObject *exception = PyErr_Occurred();
